...
class Matters(models.Model):
    folder = models.ForeignKey(CaseFolder, on_delete=models.CASCADE, null=True)
    matter_id = models.CharField(max_length=24, blank=True)
    matter_qrcode = models.ImageField(upload_to='qr_codes', blank=True, null=True)
    ...
    def save(self, *args, **kwargs):
        if not self.matter_id:
            self.matter_id = str(uuid.uuid4()).replace("-", "")[:24].lower()

        #generate QRCode

            qrcode_img = qrcode.make(self.matter_id)
            canvas = Image.new('RGB', (qrcode_img.pixel_size, qrcode_img.pixel_size), 'white')
            canvas.paste(qrcode_img)
            fname = f'qr_code-{self.matter_title}.png'
            buffer = BytesIO()
            canvas.save(buffer, 'PNG')
            self.matter_qrcode.save(fname, File(buffer), save=False)
            canvas.close()
...
class Matter_Applicant(models.Model):
    ...
    main_applicant = models.CharField(max_length=10, blank=True, choices=YESNO, default="Y")
